...
public:
  ...
  static uint32_t read_code_point(InputIterator& in_itr)
  {
    ...
    const uint8_t cu1(static_cast<uint8_t>(*in_itr)); ++in_itr;
    if (cu1 < 0x80) return static_cast<uint32_t>(cu1);
    else if ((cu1 >> 5) == 0x6)
    {
      const uint8_t cu2(static_cast<uint8_t>(*in_itr)); ++in_itr; check_continuation(cu2);
      return ((static_cast<uint32_t>(cu1) << 6) & 0x7ff) + (cu2 & 0x3f);
    }
    else if ((cu1 >> 4) == 0xe)
    {
      const uint8_t cu2(static_cast<uint8_t>(*in_itr)); ++in_itr; check_continuation(cu2);
      const uint8_t cu3(static_cast<uint8_t>(*in_itr)); ++in_itr; check_continuation(cu3);
    ...
    else if ((cu1 >> 3) == 0x1e)
    {
      const uint8_t cu2(static_cast<uint8_t>(*in_itr)); ++in_itr; check_continuation(cu2);
      const uint8_t cu3(static_cast<uint8_t>(*in_itr)); ++in_itr; check_continuation(cu3);
      const uint8_t cu4(static_cast<uint8_t>(*in_itr)); ++in_itr; check_continuation(cu4);
  ...
  static void write_code_point(OutputIterator& out_itr, const uint32_t cp)
  {
    ...
    if (cp < 0x80)
    {
      *out_itr = static_cast<uint8_t>(cp); ++out_itr;
    }
    else if (cp < 0x800)
    {
      *out_itr = static_cast<uint8_t>((cp >> 6) | 0xc0); ++out_itr;
      *out_itr = static_cast<uint8_t>((cp & 0x3f) | 0x80); ++out_itr;
    }
    else if (cp < 0x10000)
    {
      *out_itr = static_cast<uint8_t>((cp >> 12) | 0xe0); ++out_itr;
      *out_itr = static_cast<uint8_t>(((cp >> 6) & 0x3f) | 0x80); ++out_itr;
      *out_itr = static_cast<uint8_t>((cp & 0x3f) | 0x80); ++out_itr;
    }
    else if (cp < 0x10ffff)
    {
      *out_itr = static_cast<uint8_t>((cp >> 18) | 0xf0); ++out_itr;
      *out_itr = static_cast<uint8_t>(((cp >> 12) & 0x3f) | 0x80); ++out_itr;
      *out_itr = static_cast<uint8_t>(((cp >> 6) & 0x3f) | 0x80); ++out_itr;
      *out_itr = static_cast<uint8_t>((cp & 0x3f) | 0x80); ++out_itr;
    }
    else throw std::runtime_error("UTF-8 error");
